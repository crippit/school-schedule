...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-2">
          ...
          <input type="date" [ngModel]="sched.startDate()" (ngModelChange)="sched.startDate.set($event)" 
                 class="w-full px-3 py-2 border border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
        </div>
        ...
        <!-- Cycle Config -->
        <div class="space-y-2">
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Days in Cycle</label>
          <input type="number" min="1" max="20" [ngModel]="sched.cycleLength()" (ngModelChange)="sched.cycleLength.set(+$event)"
                 class="w-full px-3 py-2 border border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
        </div>

        <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">First Day is Cycle Day</label>
            <input type="number" min="1" [max]="sched.cycleLength()" [ngModel]="sched.startCycleDay()" (ngModelChange)="sched.startCycleDay.set(+$event)"
                   class="w-full px-3 py-2 border border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
            <p class="text-xs text-slate-500 dark:text-slate-400">The cycle number for {{ sched.startDate() }}</p>
        </div>

        <div class="space-y-2">
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Periods per Day</label>
          <input type="number" min="1" max="20" [ngModel]="sched.periodsPerDay()" (ngModelChange)="sched.updatePeriodCount(+$event)"
                 class="w-full px-3 py-2 border border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
        </div>

        <!-- Logic -->
        <div class="col-span-1 md:col-span-2 space-y-3 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
          <span class="block text-sm font-medium text-slate-700 dark:text-slate-300">Holiday Logic</span>
          <div class="flex flex-col sm:flex-row gap-4">
            <label class="inline-flex items-center">
              ...
              <span class="ml-2 text-slate-700 dark:text-slate-300">
                ...
                <span class="text-xs text-slate-500 dark:text-slate-400">Day 1 -> Holiday -> Day 2</span>
              </span>
            </label>
            <label class="inline-flex items-center">
              <input type="radio" class="form-radio text-indigo-600 focus:ring-indigo-500 h-5 w-5" 
              ...
              <span class="ml-2 text-slate-700 dark:text-slate-300">
                ...
                <span class="text-xs text-slate-500 dark:text-slate-400">Day 1 -> Holiday -> Day 3</span>
              </span>
            </label>
          </div>
        </div>
        ...
        <div class="col-span-1 md:col-span-2 space-y-2">
             ...
             <div class="flex gap-2">
               <input type="text" [(ngModel)]="newRoom" (keyup.enter)="addRoom()" placeholder="e.g. 101, Gym, Lab A" class="flex-1 px-3 py-2 border border-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 dark:placeholder-slate-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
             ...
             @if(sched.rooms().length > 0) {
               <div class="flex flex-wrap gap-2 mt-2 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-700">
      ...
      <!-- Bell Schedule -->
      <div class="mt-8">
         <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Bell Schedule</h3>
         <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
              <thead class="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 uppercase text-xs">
                <tr>
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                @for (period of sched.bellSchedule(); track $index) {
                  <tr>
                    <td class="px-4 py-2 font-medium text-slate-700 dark:text-slate-200">Period {{ $index + 1 }}</td>
                    <td class="px-4 py-2">
                       <input type="time" [(ngModel)]="period.start" class="px-2 py-1 border border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
                    </td>
                    <td class="px-4 py-2">
                       <input type="time" [(ngModel)]="period.end" class="px-2 py-1 border border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 rounded focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
                    </td>
                  </tr>
                }
              </tbody>
